...
const ACTION_RE: Array<[ActionType, RegExp]> = [
  ...
  // E417：裸「框架」易误伤「Tauri 框架/软件框架」等开源栈问句（无附件时无路由 → must_clarify）；
  // 文档结构抽取保留「结构/大纲/目录/…」，「框架」仅在文档语境下命中。
  ['extract_structure', /结构|大纲|目录|拆解|分节|章节|(?:文档|文章|需求|说明书).{0,8}框架/],
  // E417：软件框架/架构/技术栈名词堆叠 → qa，供 R016 web_search（bench E19 等无 GitHub 链接问句）
  ['qa', /(?:框架|runtime|sdk).{0,24}(?:架构|技术栈|原理|组成)|(?:架构).{0,12}(?:技术栈)/i],
