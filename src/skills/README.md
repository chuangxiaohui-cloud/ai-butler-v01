...
| `office-daily/` | 办公日常 | 考勤表模板 / CSV+xlsx+xlsm+xls+xlsb 占比 / 回复邮件/会议邀请草稿 / 图片压缩+格式转换（含 AVIF/TIFF，HEIC 尽力解码） / 图片 OCR 文字提取 / 文档排版（docx/md/txt/pdf/doc）/ Word↔PDF / PDF 合并+加密+压缩（含图片型降采样） / PPT / 主动提醒（设置/列出/取消） | openpyxl + python-docx + python-pptx + Pillow + pypdf（+PyMuPDF/ffmpeg 可选）+ Excel/Word COM（Python）+ 可选 LLM |
