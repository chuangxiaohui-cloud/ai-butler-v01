...
| `src/mcp/` | MCP 子 Agent 运行契约与工具适配；Keil、VS Code、STM32-GCC、KiCad ERC 与 LTspice 只读链已接入并有真实 stdio 夹具证据（E401-E410），受 schema、白名单、沙箱与 [P-154] 约束；E411 flash/串口能力与风险契约（设备白名单/固件摘要/逐次确认/串口默认只读/审计）；E420/E422/E423 受控 flash 驱动（门禁+`executeFlash`+白名单工具模板含 openocd/dfu-util/jlink）；E421 受控串口只读驱动（门禁+`executeSerialRead`+端口/波特率白名单，默认可注入 reader、默认拒绝打开）；E425 生产侧 serialport 只读注入（optionalDependency 动态加载，CLI/gateway/IM；禁止 write）；E426 设备白名单 CLI（`device:auth`）；E427 硬件审计只读 CLI（`hardware:audit`）；E428 固件摘要只读 CLI（`firmware:digest`）；E429 硬件门禁预检 CLI（`hardware:gate`，只裁决）；E424 烧录工作流节点（`hardware_flash`：计划指纹+高风险批准+本地执行，批准≠perFlashConfirmed；串口不进工作流）；E412 工作流计划指纹持久化与批准恢复校验；E413/E418 KiCad 有界原理图与 PCB 编辑（项目事务）与 LTspice 固定批仿真（高风险确认）；E419 LTspice 批开关白名单（`-ascii`/`-alt`）；E414 动作验收入口（fixture/business + [P-10] 差距复验）；不含自由布线/任意改铜、任意自定义仿真参数或串口写驱动 |
