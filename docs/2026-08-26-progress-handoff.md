...
15. **E251 市场 Skill 安全输入通道**（Phase 1 续）：manifest 声明 `input:'query'` → `MarketSkillRunner.run(name, { input })` 把查询（有界 4KB）写入沙箱 `input.txt`，步骤 `@input` 替换为文件路径（用户文本不进命令行，无注入面）；pipeline 直连传 `{ input: prepared.cleanQuery }`；CLI `--query` 带参；首个带参精选包 route-query（意图路由判题）安装并冒烟成功。新增单测 7 条 + 集成 1 条（INT-MARKET-005）；`sandbox/` 入 `.gitignore`；`maturity:check` 用户累积 Skill 4→5。
15b. **E252 Agent 浏览器操作需求增补**（Phase 1 续）：v2.5 需求新增 §4.1.5「浏览器操作（读 + 交互双模）」——Agent 可像 Codex/browser-use 一样操作浏览器（导航/点击/输入/表单/下载），动作白名单（§10.2）+ 高风险审批双闸 + SSRF 复用 + 页面观察归 untrusted_data；新增 [P-124]/[P-125]/[P-126]（provisional）；参考 browser-use 借思想已登记 borrowed-designs.md；本轮只落需求，代码实现后续迭代；browser-use 参考 4 点（DOM 观察策略/动作白名单/消息与 token 管理/验收基准）已全部落实到 §4.1.5；落地形态明确为受限的「浏览器操作 Skill」（域名白名单 + 动作白名单 + 用户确认，非通用 agent，§8.2.3 `domains` 声明）。需求计划 `docs/plans/2026-08-26-browser-agent-operation.md`；实现计划 `docs/plans/2026-08-26-browser-operation-implementation.md`（安全 TDD 先行 A1-A13，模块落点 + 依赖序）。
15c. **ego-lite 参考调研（并入 E252）**：用户所给 ego-lite/ego-lite 404，实为 citrolabs/ego-lite（MIT，13.7k star，macOS-only，Windows/Linux 在 roadmap）。借思想 4 点并入 E252 实现计划（独立 Space/登录态继承、代码底座组合多步、语义+视觉双工作流、深嵌套 iframe 快照）；不借整浏览器底座与 js/cdp 任意求值（与 §10 动作白名单冲突）；已登记 borrowed-designs.md §2.11。
