...
15b. **E252 Agent 浏览器操作需求增补**（Phase 1 续）：v2.5 需求新增 §4.1.5「浏览器操作（读 + 交互双模）」——Agent 可像 Codex/browser-use 一样操作浏览器（导航/点击/输入/表单/下载），动作白名单（§10.2）+ 高风险审批双闸 + SSRF 复用 + 页面观察归 untrusted_data；新增 [P-124]/[P-125]/[P-126]（provisional）；参考 browser-use 借思想已登记 borrowed-designs.md；本轮只落需求，代码实现后续迭代；browser-use 参考 4 点（DOM 观察策略/动作白名单/消息与 token 管理/验收基准）已全部落实到 §4.1.5。计划 `docs/plans/2026-08-26-browser-agent-operation.md`。
