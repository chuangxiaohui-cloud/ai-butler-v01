...
## 10. v2.5 交付后 owner 拍板（2026-08-30 晚，E291 后续）

- ✅ **metrics 25 行入库**：owner 同意 `bench/search-metrics.jsonl` 的 search:smoke 指标（原 15 行 + 本轮 test:all 复跑追加 10 行）作为 v2.5 性能基线，随本次发版一起 commit（40dca3c）；已核对无敏感/脏数据（全部为 STM32 基准 query + 分引擎时延字段）。
- ✅ **SSRF 最小防护入 v2.5（E292）**：协议白名单（http/https）+ 内网 IP 段黑名单（127.0.0.1 / 10.x / 172.16-31.x / 192.168.x / 169.254.x）+ 禁 30x 重定向；完整域名白名单留 v2.6（roadmap B4）。
