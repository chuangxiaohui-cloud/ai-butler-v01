...
- 遗留加强项（不回退通过结论）：P-12 本轮答案未做 C.2 人工重评。
- **E19 路由误澄清**：已由 **E417** 修复（`routeV2` → `direct`/`web_search`）；全量 v02a 未重跑。
...
| P-12 | `bench:v02a` 31 条有响应（`bench/v02a-report.md`） | 复跑当时 E19 `must_clarify`（已由 E417 修）；旧 `v02a-scores.json` 未对本轮重评 |
...
1. ~~修 E19 `must_clarify`~~ → **E417 已修**（计划 `docs/plans/2026-09-17-e19-must-clarify-fix.md`）；可选再跑全量 v02a。
2. 可选：按 C.2 对本轮 v01/v02a 答案人工回填。
3. 自由 PCB / 自定义仿真 / 真实 flash·串口仍为后续能力，不在 [P-10] 范围内虚报完成。
