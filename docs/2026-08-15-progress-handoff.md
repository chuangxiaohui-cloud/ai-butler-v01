...
> 当前分支：`v0.2b`｜最新提交：`54764e1`｜Gitee 与 GitHub 已同步。
...
28. **v1.0 三栏 UI 原型（E98）**：新增独立 Vite + React 工程 `ui/prototype`，实现三栏 × Ask/Craft/Plan、工程开发三列布局、内置终端/浏览器、证据链与轻量反馈；按参考截图重排为深色 AI 工作台风格；提问框左下新增 `+` 上传菜单（图片/文件/工程文件夹）与 Ask/Craft/Plan，聊天输入框支持直接粘贴图片，提问框右下角提供 DeepSeek/MiniMax/智谱 模型切换器；模型名已按官方文档核对；开发服务器 `http://127.0.0.1:5173/`，详见 `docs/plans/2026-08-15-three-column-ui-prototype.md`。
29. **魔鬼训练 v2.5 审阅与全量跑分（E99）**：审阅 `AI-Agent_魔鬼训练_v2.5_整理版`（122 条，6 套卷），新增 `npm run bench:devil-v25` 批量跑分脚本并全量跑完；整理版乱码已清零（重写 P01-P05/C01-C05/E32-E34，删 E31 增 E35）；owner 独立评分已定稿（`AI-Agent_魔鬼训练_v2.5_AI独立评分表.xlsx`）：均分 1.28，≥2 分 58 条，0 分 43 条，分卷 E 2.00 唯一及格、P/C 最弱；0 分大量为系统级故障（missing_time/create 排期选项/溺水救援误匹配），另有路由误判与紧急模板误触；详见 `docs/plans/2026-08-15-devil-training-v25-review.md`。
...
5. 魔鬼训练 v2.5 定稿基线已建立（均分 1.28）；优先修“路由误判 + 紧急模板误触 + 黑话/记忆/Skill 调用”，再拆 A/B 套（约 93 条单发 + 记忆依赖题多轮）重跑。
