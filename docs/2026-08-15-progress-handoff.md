...
28. **v1.0 三栏 UI 原型（E98）**：新增独立 Vite + React 工程 `ui/prototype`，实现三栏 × Ask/Craft/Plan、工程开发三列布局、内置终端/浏览器、证据链与轻量反馈；按参考截图重排为深色 AI 工作台风格；提问框左下新增 `+` 上传菜单（图片/文件/工程文件夹）与 Ask/Craft/Plan，聊天输入框支持直接粘贴图片，提问框右下角提供 DeepSeek/MiniMax/智谱 模型切换器；开发服务器 `http://127.0.0.1:5173/`，详见 `docs/plans/2026-08-15-three-column-ui-prototype.md`。
