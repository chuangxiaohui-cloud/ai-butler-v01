...
> 当前分支：`v0.2b`｜最新提交：`89fb209`｜Gitee 已同步；GitHub 待网络恢复后补推。

## 当前状态

- `npm run build` 通过；`npm run test:all` 全绿：单测 244/244 + 集成 17/17。
...
2. 检查 datasheet 下载挑选逻辑：`STM32F103C8T6.pdf` 实际是 DNV 认证证书，需确认立创页面下载是否可能挑错文件。
3. 继续用 `npm run route:feedback` 攒 accept/reject 样本，够 10 条后跑 `npm run route:apply-calibration`。
4. 需要用户配合的浏览器项：关掉 QQ浏览器 → `npm run browser:launch -- qq` → `npm run browser:cdp -- 9222`，验证登录态自动兜底。
5. 继续按 `push:hosts` 流程同步后续改动。
6. 强时效查询可继续增强：把“载人航天小喇叭”/中国载人航天工程办公室官方发布登记为航天状态权威跟踪源，进一步压缩“暂无可靠更新”场景。
