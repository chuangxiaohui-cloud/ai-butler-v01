...
> 当前分支：`v0.2b`｜最新提交：`90e539f`｜Gitee 与 GitHub 已同步。
...
29. **魔鬼训练 v2.5 审阅与全量跑分（E99）**：审阅 `AI-Agent_魔鬼训练_v2.5_整理版`（122 条，6 套卷），新增 `npm run bench:devil-v25` 批量跑分脚本并全量跑完；整理版乱码已清零（重写 P01-P05/C01-C05/E32-E34，删 E31 增 E35）；人工对照“预期行为”评 0-3 分：均分 1.40，≥2 分 65 条，0 分 45 条；主要失分是路由误判（plan/write_doc/send_message 误抢回答型意图）与紧急模板误触（造核弹/rm -rf/破解 WiFi 均返回触电急救话术）；详见 `docs/plans/2026-08-15-devil-training-v25-review.md`。
...
5. 魔鬼训练 v2.5 整理版已建立基线；优先修“路由误判 + 紧急模板误触”，再拆 A/B 套（约 93 条单发 + 记忆依赖题多轮）重跑。
