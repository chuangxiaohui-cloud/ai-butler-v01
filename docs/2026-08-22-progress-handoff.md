...
> 当前分支：`v0.2b`｜E177-E184 已提交（HEAD=`e845b74`），E185 已实现待提交。上一份交接见 `docs/2026-08-21-progress-handoff.md`。
...
- 全量单测 539/539 通过 + 1 条 fitz 特性门控用例按环境跳过、集成 17/17 全绿、doc-lint 0 FAIL 0 WARN（附录 A 949/950）（E185 收尾复核）。
...
- 已提交：E177-E184（HEAD=`e845b74`）；E185 已实现待提交（单测/集成/doc-lint 全绿）。
- 提交前请勿包含根目录 `.codex-*.cjs`（已 gitignore）、`data/` 临时文件与合成样本（已清理）。

## 明天继续（按优先级）
1. 下期迭代：跨页拼接的启发式常量复核（表头匹配阈值 70% 与 PDF 渲染 DPI=200），并用真实扫描
   件 PDF（带倾斜/透字）样本验证；若真实样本表头 OCR 有噪声，可给表头匹配加 bbox/span 级证据。
