...
| P-124 | 浏览器操作单任务最大动作数 | 30步 | numeric | provisional@2026-08-26 | | 
| P-125 | 浏览器操作单步执行超时 | 15000ms | numeric | provisional@2026-08-26 | | 
| P-126 | 浏览器操作每步 DOM 快照上限 | 8000字符 | numeric | provisional@2026-08-26 | | 
...
> 方法：示例 Skill `datasheet-fetch`（已授权域名 szlcsc.com / st.com）+ 本机 Edge + 真实联网；动作链 goto(搜索) → click(数据手册) → wait → download(选择器 `a[href$=".pdf"]`，运行期 `resolveHref` 解析真实 URL，解析结果仍过域名白名单/SSRF/审批后置门)；落盘 `data/browser-downloads/`（运行时产物，git 忽略不提交）。
...
> 结论：2 正例验证「选择器动态解析 → 域名/SSRF/审批后置门 → 真实下载」全链成立；B3 反例验证「页面内合法链接但站外域名同样被白名单拦截」，安全边界未因动态解析被绕过。样本阈值（§0.3 样例 n≥阈值，必要非充分）与 [P-124]/[P-125]/[P-126] 定稿待 [P-10] 验收门（owner 签认）；后续按 §4.1.5 场景扩充（器件参数对比 / 表格填写）。
