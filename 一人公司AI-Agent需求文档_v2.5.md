...
> 结论：2 正例验证「选择器动态解析 → 域名/SSRF/审批后置门 → 真实下载」全链成立；B3 反例验证「页面内合法链接但站外域名同样被白名单拦截」，安全边界未因动态解析被绕过。样本阈值（§0.3 样例 n≥阈值，必要非充分）：owner 2026-08-27 以 n=3 签认推进，[P-124]/[P-125]/[P-126] 已定稿（见附录 A 定稿记录）；后续按 §4.1.5 场景继续扩充样例集（器件参数对比 / 表格填写）。
