...
| P-128 | Stage 5 网页正文单篇注入 prompt 上限 | 3000字符 | numeric | 定稿 | | 
...
> - P-14 是约束（定稿）；P-13/P-15 是分配（定稿，调值需附录 A 登记），和不得超 P-14
...
- news → 日期+引述动词；github_analysis → 标识符
...
**信号 C：证据多样性约束（Top-K 后替换）**——同域名且标题/正文 token Jaccard ≥ [P-131] 视为同质簇，top-K 内从剩余候选中替换异质高分项，保证覆盖 ≥2 个语义簇；纯集合运算，零领域知识
