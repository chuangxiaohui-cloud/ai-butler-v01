...
- **验证**：`npm run build` 通过；按参考截图重排为深色 AI 工作台风格（近黑面板 + 蓝色强调），提问框左下新增 `+` 上传菜单（图片/文件/工程文件夹）与 Ask/Craft/Plan，聊天输入框支持直接粘贴图片，提问框右下角提供 DeepSeek/MiniMax/智谱 模型切换器；Playwright 检查工程/知识/移动三个视口无横向溢出、控制台 0 错误；开发服务器 `http://127.0.0.1:5173/`；详见 `docs/plans/2026-08-15-three-column-ui-prototype.md`。
