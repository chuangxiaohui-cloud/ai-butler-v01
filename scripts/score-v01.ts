...
/**
 * [P-07] 评分判定（WP11 收尾）
 * 读取 bench/v01-scores.json（模板见 bench/v01-scores.example.json），判定：
 * 10 条中 ≥8 条相关性 ≥2 分，且无 0 分硬答。
 */
...
const scoresPath = scoresArg
  ...
  : join(root, 'bench', 'v01-scores.json');
const queriesPath = join(root, 'bench', 'v01-queries.json');

function main(): void {
  if (!existsSync(scoresPath)) {
    console.error('缺少 bench/v01-scores.json；请复制 bench/v01-scores.example.json 并回填评分。');
    process.exit(1);
  }
  ...
  if (missing.length > 0 || unfilled.length > 0) {
    console.error(`评分不完整：缺失 ${missing.join(',') || '无'}，未填/非法 ${unfilled.map((s) => s.id).join(',') || '无'}。`);
    process.exit(1);
  }
